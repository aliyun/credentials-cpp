...
bool OAuthCredentialsProvider::refreshCredential() const {
  ...
  std::string path = (hostEnd != std::string::npos) ?
                     tokenEndpoint_.substr(hostEnd) : "/";
  ...
  std::string body = "grant_type=client_credentials"
                     "&client_id=" + Darabonba::Encode::Encoder::urlEncode(clientId_) +
  ...
  if (resp->statusCode() != 200) {
    throw Darabonba::Exception(
        OAUTH_FETCH_ERROR_MSG + " Status code is " +
  ...
  if (result.contains("error")) {
    throw Darabonba::Exception(
        OAUTH_FETCH_ERROR_MSG + " Error: " + 
        result["error"].get<std::string>() + 
        (result.contains("error_description") ? 
         ", Description: " + result["error_description"].get<std::string>() : ""));
  }

  // Extract OAuth token - typically access_token
  std::string accessToken = result["access_token"].get<std::string>();
  
  // Calculate expiration time
  int64_t expiresIn = result.contains("expires_in") ? 
                      result["expires_in"].get<int64_t>() : 3600;
  ...
  // If the response includes additional credentials (some OAuth implementations)
  ...
  if (result.contains("access_key_secret")) {
    credential_.setAccessKeySecret(result["access_key_secret"].get<std::string>());
  }
