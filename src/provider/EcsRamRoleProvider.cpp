...
EcsRamRoleProvider::EcsRamRoleProvider(
    ...
    : RefreshableProvider(behavior, strategy),
      ...
      disableIMDSv1_(config->hasDisableIMDSv1() ? config->disableIMDSv1() : false),
      shouldRefresh_(false),
      asyncUpdateEnabled_(asyncUpdateEnabled),
      connectTimeout_(config->hasConnectTimeout() ? config->connectTimeout() : DEFAULT_CONNECT_TIMEOUT),
      readTimeout_(config->hasTimeout() ? config->timeout() : DEFAULT_READ_TIMEOUT) {
  
  // 检查是否禁用了 IMDS（对应 Python 的 environment_ecs_metadata_disabled 检查）
  std::string ecsMetadataDisabled = Darabonba::Env::getEnv("ALIBABA_CLOUD_ECS_METADATA_DISABLED");
  ...
  if (!disableIMDSv1_) {
    std::string imdsv1Disabled = Darabonba::Env::getEnv("ALIBABA_CLOUD_IMDSV1_DISABLED");
...
std::string EcsRamRoleProvider::getMetadataToken() const {
  std::string url = "http://" + META_DATA_SERVICE_HOST + URL_IN_ECS_METADATA_TOKEN;
  ...
  try {
    ...
    if (resp->statusCode() != 200) {
      throw Darabonba::Exception(
          ECS_METADATA_TOKEN_FETCH_ERROR_MSG + " HttpCode=" +
...
RefreshResult EcsRamRoleProvider::doRefresh() const {
  ...
  std::string url = "http://" + META_DATA_SERVICE_HOST + 
                    URL_IN_ECS_META_DATA + roleNameToUse;
  auto req = AuthUtil::getNewRequest(url);
  ...
  if (resp->statusCode() != 200) {
    throw Darabonba::Exception(
        ECS_METADATA_FETCH_ERROR_MSG + " HttpCode=" +
        std::to_string(resp->statusCode()));
  }
  ...
  if (contentCode != "Success") {
    throw Darabonba::Exception(
        ECS_METADATA_FETCH_ERROR_MSG + " Code=" + contentCode);
  }
...
int64_t EcsRamRoleProvider::getPrefetchTime(int64_t expiration) const {
  int64_t now = getCurrentTime();
